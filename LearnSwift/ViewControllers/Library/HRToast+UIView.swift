    ...
    */
    ...
    func viewForMessage(msg: String?, title: String?, image: UIImage?) -> UIView? {
        ...
        if msg != nil {
            ...
            var textSize:CGRect = msg!.stringRectWithFontSize(HRToastFontSize, width: maxSizeMessage.width)
            var expectedWidth = textSize.width
            if(expectedWidth > maxSizeMessage.width){
                expectedWidth = maxSizeMessage.width
            }
...
extension String {
    ...
    func stringRectWithFontSize(fontSize: CGFloat,width: CGFloat) -> CGRect {
        var font = UIFont.systemFontOfSize(fontSize)
        var size = CGSizeMake(width, CGFloat.max)
        var paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineBreakMode = .ByWordWrapping;
